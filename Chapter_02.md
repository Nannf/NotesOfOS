...
>
> 并发的问题都是多个线程对共享数据的操作因为没有互相通信导致的不一致问题。



Q: 为什么会出现磁盘

A：内存为了读写的速度，当断电时就不在存储数据，显然不适合作为介质来存放程序和数据（这二者本质上都是二进制序列），我们需要一个容量较大的，断电之后仍然能存储数据的介质，这就是磁盘，把数据放到磁盘上的过程又叫做**持久化**。

> 操作系统中用来管理磁盘的软件被称作文件系统。它负责可靠、高效的存储用户创建的文件。

**磁盘是不会被虚拟的，他是所有线程共享的部分**，之所以如此是因为文件系统的开发者认为文件在不同的系统之间共享是很正常且普遍的事情，之前的内存和cpu是因为在不同的进程之间是没有办法共享的。

Q: 常见的操作磁盘的系统调用有哪些

A: open() 打开文件，然后创建； write() 将数据写入文件； close() 关闭文件。这些系统调用在文件系统中，至于详细的细节在后文有叙述
